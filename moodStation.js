...
window.renderMoodStation = function(containerId = 'mood-station-container') {
    ...
    container.innerHTML = `
        <section class="mb-8 bg-slate-900/90 border border-slate-800 rounded-3xl p-4 sm:p-5 shadow-2xl relative overflow-hidden backdrop-blur-xl">
           <div class="space-y-3">
                <!-- Tiêu đề Trạm Sạc -->
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                    <div class="inline-flex items-center gap-2.5 bg-slate-900/90 border border-emerald-500/30 px-4 py-2.5 rounded-2xl shadow-lg">
                        <span class="w-3 h-3 rounded-full bg-emerald-400 animate-ping shrink-0"></span>
                        <span class="text-xs sm:text-sm font-black text-emerald-400 tracking-wider uppercase cyber-header-glow">
                            🔋 TRẠM SẠC CẢM XÚC
                        </span>
                    </div>

                    <!-- Các nút chọn tâm trạng (Tối ưu rớt dòng gọn gàng trên mobile) -->
                    <div class="flex flex-wrap items-center gap-1.5 sm:gap-2 w-full sm:w-auto">
                        ${Object.keys(MOOD_DATA).map(key => {
                            ...
                            return `<button onclick="handleMoodSelect('${key}')" class="px-2.5 sm:px-3 py-1.5 border text-[11px] sm:text-xs font-bold rounded-xl transition-all hover:scale-105 cursor-pointer shadow-md ${m.btnClass} shrink-0">${m.label}</button>`;
                        }).join('')}
                    </div>
                </div>
            </div>

            <!-- Hộp hiển thị thông điệp / lời chúc -->
            <div class="my-4 p-4 sm:p-5 bg-slate-950/80 rounded-2xl border border-slate-800 relative flex items-start sm:items-center gap-3 text-left shadow-lg transition-all duration-300" id="mood-talk-box">
                <div class="text-2xl sm:text-3xl shrink-0 pt-0.5 sm:pt-0" id="talk-avatar">💡</div>
                <div class="space-y-2 flex-1 min-w-0">
                    ...
                    <div id="welcome-prompt" class="space-y-2">
                        <div class="w-full h-px bg-gradient-to-r from-transparent via-slate-700 to-transparent my-1"></div>
                        <h2 class="text-xs sm:text-base font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-emerald-400 uppercase tracking-wider">
                            Sạc năng lượng ngày mới! Tâm trạng của bạn hôm nay như thế nào?
                        </h2>
                    </div>
            ...
            <!-- Khu vực Mini-Game và Bảng điều khiển Âm nhạc -->
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-3 items-center">
                <!-- Nút mở Mini-Game -->
                <div class="lg:col-span-4 bg-slate-950/80 border border-slate-800/80 p-3 rounded-2xl flex items-center justify-between gap-2">
                    <div class="flex items-center gap-2.5 min-w-0">
                        <div class="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-gradient-to-tr from-cyan-500 via-sky-500 to-blue-600 flex items-center justify-center shadow-[0_0_15px_rgba(56,189,248,0.7)] animate-bounce shrink-0">
                            <span class="text-sm sm:text-base">🎮</span>
                        </div>
                        <span id="mini-game-name" class="text-xs font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 via-sky-300 to-blue-300 tracking-wide truncate">Mini-Game Giải Trí</span>
                    </div>
                    <button onclick="launchCurrentMoodGame()" class="px-3 py-1.5 bg-gradient-to-r from-amber-500 to-indigo-600 text-slate-950 font-black text-[11px] sm:text-xs rounded-xl shadow-md transition hover:scale-105 cursor-pointer shrink-0">
                        Chơi Ngay 🚀
                    </button>
                </div>

                <!-- 3 Ô ÂM NHẠC & THANH ĐIỀU KHIỂN (Responsive hoàn toàn) -->
                <div class="lg:col-span-8 bg-slate-950/80 border border-slate-800/80 p-3 rounded-2xl flex flex-col gap-2.5">
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                        <!-- Ô 1: Nhạc Sôi Động -->
                        <div class="flex items-center gap-2 bg-amber-500/10 border border-amber-500/30 px-2.5 py-1.5 rounded-xl min-w-0">
                        ...
                        <!-- Ô 2: Nhạc Nhẹ Nhàng -->
                        <div class="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/30 px-2.5 py-1.5 rounded-xl min-w-0">
                        ...
                        <!-- Ô 3: Âm Thanh Xanh -->
                        <div class="flex items-center gap-2 bg-cyan-500/10 border border-cyan-500/30 px-2.5 py-1.5 rounded-xl min-w-0">
                            ...
                            <div class="overflow-hidden min-w-0 flex-1">
                                ...
                                <div id="rain-status" class="text-[9px] text-slate-400 truncate">Thiên nhiên</div>
                            </div>
                        </div>
                    </div>

                    <!-- Thanh điều hướng nhạc phụ: Prev, Stop, Next -->
        ...
        <div id="mood-custom-modal" class="fixed inset-0 bg-slate-950/90 backdrop-blur-md z-[9999] flex items-center justify-center p-3 sm:p-5 hidden">
            <div class="bg-slate-900 border-2 border-indigo-500/50 rounded-3xl max-w-4xl w-full p-4 sm:p-6 relative shadow-2xl text-center space-y-3 max-h-[90vh] overflow-y-auto">
